...
def get_sale_cars():
    ...
    try:
        ...
        if user_role['Allow_ext_car_view'] == True:
            cars_db = conn.get_trade_car(sortby=sortby, all=True)
        else:
        ...
        for car in cars_db:
            ...
            car_new["car_img"] = (
                f'../cars_imgs/trade_cars/{car["id"]}/{conn.get_trade_car_pics(car["id"])[0]["pic_name"]}'
            )
            car_new["status"] = car["status"]
            cars.append(car_new)
    ...
    return fl.render_template("sale_cars.html", cars=cars, user_role=user_role)
